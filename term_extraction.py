...
def extract_phrasesfrom_textrank(corpus):
    ...
    celeb_list = ['Bradley Cooper', 'Chris Kyle', 'Clint Eastwood', 'bradley cooper', 'bradley', 'cooper', 'chris kyle',
                  'chris', 'kyle', 'clint eastwood', 'clint', 'eastwood']
    ...
    for index, row in new_df_tweet.iterrows():
        if any(celeb in row['keywords'] for celeb in celeb_list):
            cleaned_df_tweet = cleaned_df_tweet.append({'sentences': row['text'], 'keywords': row['keywords']},
                                                       ignore_index=True)
...
def fetch_data_from_concept_net(data, relations, use_proxy=True):
    ...
    concept_net_request_iris = dict(
        map(lambda x: (x, "http://api.conceptnet.io/c/en/" + x + '?offset=0&limit=1000'), data))
    ...
    for word, concept_net_request_iri in concept_net_request_iris.items():
        ...
        if use_proxy:
            concept_net_response = requests.get(concept_net_request_iri,
                                                proxies={'http': "http://10.135.0.29:8080"}).json()
        else:
        ...
        if concept_net_response is not None:
            ...
            while 'view' in list(tmp_response.keys()):
                if 'nextPage' in list(tmp_response['view'].keys()):
                    if use_proxy:
                        tmp_response = requests.get("http://api.conceptnet.io" + tmp_response['view']['nextPage'],
                                                    proxies={'http': "http://10.135.0.29:8080"}).json()
                    else:
                        tmp_response = requests.get(
                            "http://api.conceptnet.io" + tmp_response['view']['nextPage']).json()
    ...
    for word, concept_net_response in concept_net_responses.items():
        for relation in relations:
            end_tags = [tmp['start']['label'] for tmp in concept_net_response['edges'] if tmp['@type'] == 'Edge' and
                        tmp['rel']['label'] == relation and tmp['start']['language'] == 'en' and tmp['end'][
                            'language'] == 'en']
